...
public class EmployeeDAOImpl implements EmployeeDAO {
    ...
    @Override
    public void updateEmployee(Integer id, Employee employeeUpdate) {
        try (PreparedStatement statement = connection.prepareStatement(
                "UPDATE employee SET first_name = (?), last_name = (?), gender = (?), age = (?), city_id = (?) WHERE id = (?)")) {
            statement.setString(1, employeeUpdate.getFirstName());
            statement.setString(2, employeeUpdate.getLastName());
            statement.setString(3, employeeUpdate.getGender());
            statement.setInt(4, employeeUpdate.getAge());
            statement.setInt(5, employeeUpdate.getCity().getId());
